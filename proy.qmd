...
gantt
    ...
    Definición de variables relevantes          :a2, 2026-01-10, 10d
    Creación de la herramienta de recolección   :a3, 2026-01-16, 10d

    section Análisis de Datos
    Análisis descriptivo de los datos           :b1, 2026-02-01, 9d
    Análisis de interacción                     :b2, 2026-02-06, 11d

    section Redacción y Preparación
    Redacción de resultados e informe final     :c1, 2026-02-18, 9d
```
...
```{R}
# ==============================================================================
# ANÁLISIS COVID-19 - CÓDIGO CORREGIDO PARA INSERTAR EN QUARTO
# ==============================================================================
# Instrucciones: Copia cada sección en tu documento .qmd donde corresponda

# ==============================================================================
# OBJETIVO 1: Score de riesgo e Índice de gravedad
# ==============================================================================
...
tabla_obj1 <- DATOS_FINALES %>%
  summarise(
    Variable = c("SCORE_RIESGO_COVID", "INDICE_GRAVEDAD"),
    Mínimo = c(min(SCORE_RIESGO_COVID, na.rm = TRUE), 
               min(INDICE_GRAVEDAD, na.rm = TRUE)),
    Q1 = c(quantile(SCORE_RIESGO_COVID, 0.25, na.rm = TRUE),
           quantile(INDICE_GRAVEDAD, 0.25, na.rm = TRUE)),
    Mediana = c(median(SCORE_RIESGO_COVID, na.rm = TRUE),
                median(INDICE_GRAVEDAD, na.rm = TRUE)),
    Media = c(mean(SCORE_RIESGO_COVID, na.rm = TRUE),
              mean(INDICE_GRAVEDAD, na.rm = TRUE)),
    Q3 = c(quantile(SCORE_RIESGO_COVID, 0.75, na.rm = TRUE),
           quantile(INDICE_GRAVEDAD, 0.75, na.rm = TRUE)),
    Máximo = c(max(SCORE_RIESGO_COVID, na.rm = TRUE),
               max(INDICE_GRAVEDAD, na.rm = TRUE)),
    Desv_Est = c(sd(SCORE_RIESGO_COVID, na.rm = TRUE),
                 sd(INDICE_GRAVEDAD, na.rm = TRUE))
  )

kable(tabla_obj1, digits = 3, 
      caption = "Tabla 1. Estadísticos descriptivos de Score de Riesgo e Índice de Gravedad")

# ------------------------------------------------------------------------------
# GRÁFICO 1.1: Histogramas
# ------------------------------------------------------------------------------

# Histograma - Score de Riesgo
...
# ------------------------------------------------------------------------------
# GRÁFICO 1.2: Boxplots
# ------------------------------------------------------------------------------

# Boxplot - Score de Riesgo
ggplot(DATOS_FINALES, aes(y = SCORE_RIESGO_COVID)) +
  geom_boxplot(fill = "#3498db", alpha = 0.7, outlier.color = "#e74c3c") +
  labs(
    title = "Diagrama de Caja: Score de Riesgo COVID",
    y = "Score de Riesgo COVID"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 14),
...
# Boxplot - Índice de Gravedad
ggplot(DATOS_FINALES, aes(y = INDICE_GRAVEDAD)) +
  geom_boxplot(fill = "#e74c3c", alpha = 0.7, outlier.color = "#3498db") +
  labs(
    title = "Diagrama de Caja: Índice de Gravedad",
    y = "Índice de Gravedad"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 14),
...
# ==============================================================================
# OBJETIVO 2: n_comorbilidades × TIPO_PACIENTE
# ==============================================================================

# ------------------------------------------------------------------------------
# Crear etiquetas descriptivas (solo una vez)
# ------------------------------------------------------------------------------

DATOS_FINALES <- DATOS_FINALES %>%
  mutate(TIPO_PACIENTE_LABEL = case_when(
    TIPO_PACIENTE == 1 ~ "Ambulatorio",
    TIPO_PACIENTE == 2 ~ "Hospitalizado",
    TRUE ~ as.character(TIPO_PACIENTE)
  ))

# ------------------------------------------------------------------------------
# TABLA 2: Resumen de N_COMORBILIDADES por TIPO_PACIENTE
# ------------------------------------------------------------------------------

tabla_obj2 <- DATOS_FINALES %>%
  group_by(TIPO_PACIENTE_LABEL) %>%
  summarise(
    N = n(),
    Mínimo = min(N_COMORBILIDADES, na.rm = TRUE),
    Q1 = quantile(N_COMORBILIDADES, 0.25, na.rm = TRUE),
    Mediana = median(N_COMORBILIDADES, na.rm = TRUE),
    Media = mean(N_COMORBILIDADES, na.rm = TRUE),
    Q3 = quantile(N_COMORBILIDADES, 0.75, na.rm = TRUE),
    Máximo = max(N_COMORBILIDADES, na.rm = TRUE),
    Desv_Est = sd(N_COMORBILIDADES, na.rm = TRUE)
  ) %>%
  rename(`Tipo de Paciente` = TIPO_PACIENTE_LABEL)

kable(tabla_obj2, digits = 2,
      caption = "Tabla 2. Número de comorbilidades según tipo de paciente")

# ------------------------------------------------------------------------------
# GRÁFICO 2: Boxplot por grupo
# ------------------------------------------------------------------------------

ggplot(DATOS_FINALES, aes(x = TIPO_PACIENTE_LABEL, 
                          y = N_COMORBILIDADES,
                          fill = TIPO_PACIENTE_LABEL)) +
  geom_boxplot(alpha = 0.7, outlier.color = "#e74c3c") +
  scale_fill_manual(values = c("Ambulatorio" = "#2ecc71", 
                                "Hospitalizado" = "#e67e22")) +
  labs(
...
# ==============================================================================
# OBJETIVO 3: SEXO × CLASIFICACION_FINAL
# ==============================================================================
...
DATOS_FINALES <- DATOS_FINALES %>%
  mutate(
    ...
    CLASIFICACION_LABEL = case_when(
      CLASIFICACION_FINAL == 1 ~ "Confirmado clínico-epidemiológico",
      CLASIFICACION_FINAL == 2 ~ "Confirmado por comité",
      CLASIFICACION_FINAL == 3 ~ "SARS-CoV-2 confirmado",
...
# ------------------------------------------------------------------------------
# TABLA 3a: Tabla de contingencia (frecuencias absolutas)
# ------------------------------------------------------------------------------

# Opción 1: Con tidyr::pivot_wider (requiere library(tidyr))
# tabla_obj3_abs <- DATOS_FINALES %>%
#   count(SEXO_LABEL, CLASIFICACION_LABEL) %>%
#   pivot_wider(names_from = CLASIFICACION_LABEL, values_from = n, values_fill = 0)

# Opción 2: Con tabla base de R (SIN tidyr)
tabla_obj3_abs <- table(DATOS_FINALES$SEXO_LABEL, DATOS_FINALES$CLASIFICACION_LABEL)

kable(tabla_obj3_abs,
      caption = "Tabla 3a. Tabla de contingencia: Sexo × Clasificación Final (frecuencias absolutas)")

# ------------------------------------------------------------------------------
# TABLA 3b: Tabla de contingencia (porcentajes por fila)
# ------------------------------------------------------------------------------

# Calcular porcentajes por fila
tabla_obj3_pct <- prop.table(tabla_obj3_abs, margin = 1) * 100

kable(tabla_obj3_pct, digits = 1,
      caption = "Tabla 3b. Distribución porcentual por sexo (%)")

# ------------------------------------------------------------------------------
# GRÁFICO 3.1: Barras agrupadas
# ------------------------------------------------------------------------------

ggplot(DATOS_FINALES, aes(x = CLASIFICACION_LABEL, fill = SEXO_LABEL)) +
  geom_bar(position = "dodge", alpha = 0.8) +
  scale_fill_manual(values = c("Mujer" = "#e91e63", "Hombre" = "#2196f3")) +
  labs(
    title = "Clasificación Final según Sexo",
...
# ------------------------------------------------------------------------------
# GRÁFICO 3.2: Barras apiladas (100%)
# ------------------------------------------------------------------------------

ggplot(DATOS_FINALES, aes(x = SEXO_LABEL, fill = CLASIFICACION_LABEL)) +
  geom_bar(position = "fill", alpha = 0.8) +
  scale_y_continuous(labels = scales::percent) +
  labs(
    title = "Proporción de Clasificación Final por Sexo",
    x = "Sexo",
    y = "Porcentaje",
    fill = "Clasificación Final"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 14),
...
# Análisis de resultados - CORREGIDO (todo en mayúscula)
