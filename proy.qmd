...
```{r}
n_casos <- table(DATOS_FINALES$TIPO_PACIENTE)
medias <- tapply(DATOS_FINALES$N_COMORBILIDADES, 
                 DATOS_FINALES$TIPO_PACIENTE, 
                 mean, na.rm = TRUE)
medianas <- tapply(DATOS_FINALES$N_COMORBILIDADES, 
                   DATOS_FINALES$TIPO_PACIENTE, 
                   median, na.rm = TRUE)
maximos <- tapply(DATOS_FINALES$N_COMORBILIDADES, 
                  DATOS_FINALES$TIPO_PACIENTE, 
                  max, na.rm = TRUE)

tabla_obj2_base <- data.frame(
  Tipo_Paciente = names(n_casos),
  Casos = as.numeric(n_casos),
  Media = round(as.numeric(medias), 2),
  Mediana = as.numeric(medianas),
  Máximo = as.numeric(maximos)
)

knitr::kable(tabla_obj2_base, 
             caption = "Comorbilidades por tipo de paciente",
             row.names = FALSE)
```
...
```{r}
if(!is.factor(DATOS_FINALES$TIPO_PACIENTE)) {
  DATOS_FINALES$TIPO_PACIENTE <- as.factor(DATOS_FINALES$TIPO_PACIENTE)
}

niveles <- levels(DATOS_FINALES$TIPO_PACIENTE)
colores <- c("#FFD700", "#2E8B57")  


par(mar = c(5, 4, 4, 2))  

boxplot(N_COMORBILIDADES ~ TIPO_PACIENTE,
        data = DATOS_FINALES,
        col = colores,
        boxwex = 0.6,
        main = "Distribución de Comorbilidades por Tipo de Paciente",
        xlab = "Tipo de Paciente",
        ylab = "Número de Comorbilidades",
        names = niveles,  
        cex.main = 1.2,
        font.lab = 2)
...
legend("topright",
       legend = niveles,
       fill = colores[1:length(niveles)],  
       title = "Tipo de Paciente",
       bty = "n")


text(x = 1:length(niveles),
     y = par("usr")[3] - (par("usr")[4] - par("usr")[3]) * 0.05,
     labels = paste("n =", table(DATOS_FINALES$TIPO_PACIENTE)),
     xpd = TRUE)
```

##Objetivo específico 3 ###Distribución de los casos según sexo y clasificación final ###Tabla de contingencia
...
La depuración de datos permitió trabajar con un conjunto de observaciones consistentes para el análisis. Los resultados principales muestran que:

El score de riesgo COVID y el índice de gravedad son indicadores útiles para resumir el perfil clínico de los casos, con distribuciones que evidencian la predominancia de casos de menor severidad en la muestra.

Existe una clara diferencia en comorbilidades según el tipo de atención: los pacientes hospitalizados presentan mayor carga de comorbilidades que los ambulatorios, lo que sugiere su asociación con la necesidad de hospitalización.

La distribución por sexo es similar entre las diferentes clasificaciones finales, sin evidenciar diferencias marcadas en la confirmación de casos.

Un hallazgo interesante es la correlación positiva débil entre edad y riesgo (r = 0.253), que aunque estadísticamente significativa, es menos marcada de lo esperado, posiblemente reflejando el impacto de la vacunación en poblaciones mayores durante 2021.

Limitaciones: Este análisis es descriptivo y no establece causalidad. Los índices creados son aproximaciones útiles para comparación interna, pero no equivalen a escalas clínicas validadas.
