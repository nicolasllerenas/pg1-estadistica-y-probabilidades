...
quantile(DATOS_FINALES$INDICE_GRAVEDAD, probs = c(.25, .50, .75), na.rm = TRUE)
```
...
##Objetivo específico 2 ###Riesgo Clínico vs Gravedad Real ###Tabla resumen
...
tabla_prediccion <- DATOS_FINALES %>%
  ...
  summarise(
    ...
    # Calculamos el % de pacientes que fallecieron en cada grupo para dar contexto
    Mortalidad_Pct = round(mean(ES_FALLECIDO, na.rm=TRUE) * 100, 1)
  )

knitr::kable(tabla_prediccion, col.names = c("Nivel Riesgo", "Pacientes", "Índice Gravedad Promedio", "Desviación", "Mortalidad (%)"))
```

```{r}

ggplot(DATOS_FINALES, aes(x = RIESGO_CLINICO, y = INDICE_GRAVEDAD, fill = RIESGO_CLINICO)) +
  geom_violin(trim = FALSE, alpha = 0.6) + # Forma de la distribución
  geom_boxplot(width = 0.1, fill = "white", outlier.shape = NA) + # Caja interior
  
  # Agregamos el promedio como punto rojo (La tendencia clave)
  stat_summary(fun = mean, geom = "point", shape = 18, size = 3, color = "red") +
  
  labs(
    title = "Validación Predictiva: Riesgo vs Desenlace",
    subtitle = "El punto rojo muestra cómo aumenta la gravedad promedio según el riesgo",
    x = "Nivel de Riesgo Clínico (Basado en Comorbilidades)",
    y = "Índice de Gravedad Real (0 a 1)",
    fill = "Nivel Riesgo"
  ) +
  theme_minimal() +
  scale_fill_brewer(palette = "YlOrRd") + # Amarillo a Rojo (Alerta)
