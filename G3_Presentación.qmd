...
Definimos un experimento aleatorio para analizar la variable **Nivel de Riesgo Clínico** (`RIESGO_CLINICO`), calculando las probabilidades basándonos en las frecuencias observadas en la muestra de **17,714 pacientes**.

### Definición del Experimento ($\varepsilon$)

-   **Experimento:** Seleccionar al azar un paciente de la base de datos de vigilancia epidemiológica de COVID-19 (México, 2021) y observar su categoría de riesgo clínico.
-   **Variable aleatoria (**$X$): Nivel de Riesgo Clínico (Ordinal).
-   **Espacio Muestral (**$\Omega$): El conjunto de todos los posibles resultados (niveles de riesgo). $$\Omega = \{ \text{Muy bajo}, \text{Bajo}, \text{Moderado}, \text{Alto}, \text{Muy alto}, \text{Crítico} \}$$

### Eventos Atómicos

Definimos los eventos $E_i$ correspondientes a cada categoría:

-   $E_1$: El paciente tiene riesgo **Muy bajo** (0 comorbilidades).

-   $E_2$: El paciente tiene riesgo **Bajo** (1-2 comorbilidades).

-   $E_3$: El paciente tiene riesgo **Moderado** (3-4 comorbilidades).

-   $E_4$: El paciente tiene riesgo **Alto** (5-6 comorbilidades).

-   $E_5$: El paciente tiene riesgo **Muy alto** (7-8 comorbilidades).

-   $E_6$: El paciente tiene riesgo **Crítico** (9-10 comorbilidades).

### Cálculo de Probabilidades

Utilizamos la frecuencia relativa: $P(E_i) = \frac{f_i}{n}$.

```{r}
# Crear tabla de probabilidades empíricas
tabla_probabilidad <- DATOS_FINALES |>
  group_by(RIESGO_CLINICO) |>
  summarise(
    Frecuencia = n(),
    Probabilidad = n() / nrow(DATOS_FINALES)
  ) |>
  mutate(
    Porcentaje = paste0(round(Probabilidad * 100, 2), "%"),
    Evento = paste0("E", row_number())
  ) |>
  select(Evento, RIESGO_CLINICO, Frecuencia, Probabilidad, Porcentaje)

# Mostrar tabla
kbl(tabla_probabilidad, 
    col.names = c("Evento", "Nivel de Riesgo", "Frecuencia (f)", "Probabilidad P(E)", "%"),
    align = "c",
    caption = "Probabilidades Empíricas del Nivel de Riesgo") |>
  kable_styling(bootstrap_options = "striped", full_width = F)
```

**Interpretación:** El evento más probable es $E_1$ (Riesgo Muy bajo), con una probabilidad de **`r tabla_probabilidad$Porcentaje[1]`**. Esto indica que la mayoría de la población captada no presentaba comorbilidades múltiples. Por otro lado, el evento menos probable es $E_6$ (Riesgo Crítico), con una probabilidad de **`r tabla_probabilidad$Porcentaje[6]`**.

## Probabilidad Condicional

Analizamos la probabilidad de fallecer ($M$) dado un cierto nivel de riesgo clínico ($R$). Esto nos permite cuantificar la letalidad asociada a las comorbilidades.

### Definición de Eventos

-   $A$: El paciente **Fallece** (Estado = "Muerto").

-   $B_i$: El paciente pertenece al nivel de riesgo $i$ (ej. "Crítico").

### Tabla de Contingencia y Cálculo

Calculamos la probabilidad condicional $P(A | B_i) = \frac{P(A \cap B_i)}{P(B_i)}$, que equivale a la proporción de fallecidos dentro de cada grupo de riesgo.

```{r}
# Tabla de contingencia (Filas: Riesgo, Col: Estado)
tabla_condicional <- table(DATOS_FINALES$RIESGO_CLINICO, DATOS_FINALES$ESTADO)

# Calcular proporciones por fila (Probabilidad Condicional)
prob_condicional <- prop.table(tabla_condicional, margin = 1)

# Convertir a Data Frame para visualizar mejor
df_condicional <- as.data.frame.matrix(prob_condicional) |>
  mutate(
    Prob_Fallecer = round(Muerto, 4),
    Prob_Sobrevivir = round(Vivo, 4)
  ) |>
  select(Prob_Sobrevivir, Prob_Fallecer)

kbl(df_condicional, 
    caption = "Probabilidad de fallecer dado el nivel de riesgo: P(Muerto | Riesgo)",
    align = "c") |>
  kable_styling(bootstrap_options = "hover", full_width = F) |>
  column_spec(3, bold = TRUE, color = "red") # Resaltar col de muerte
```

**Interpretación:** Se observa un comportamiento mixto en la probabilidad condicional. En los primeros niveles, existe una **relación directa**: la probabilidad de fallecer aumenta progresivamente desde el **Riesgo Muy Bajo** (`r df_condicional$Prob_Fallecer[1]`) hasta alcanzar su pico en el **Riesgo Alto** (`r df_condicional$Prob_Fallecer[4]`). Sin embargo, en los niveles extremos (**Muy Alto y Crítico**), la probabilidad de fallecer desciende, llegando incluso a **`r df_condicional$Prob_Fallecer[6]`** en el grupo Crítico.

> **Conclusión del análisis:** La dependencia positiva entre riesgo y letalidad se confirma claramente hasta el nivel "Alto". La caída inesperada en los niveles superiores ("Crítico") sugiere un **sesgo por tamaño de muestra pequeño** en esos grupos extremos (pocos casos registrados), lo que genera estimaciones estadísticas menos representativas que rompen la tendencia general.
