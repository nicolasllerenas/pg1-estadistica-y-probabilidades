...
### Variable de Estudio 1: Nivel de Riesgo Clínico
...
kbl(tabla_riesgo, 
    col.names = c("Nivel de Riesgo ($E_i$)", "Frecuencia ($f_i$)", "Probabilidad ($P$)", "%"),
    align = "c", caption = "Probabilidad Empírica: Riesgo Clínico") |>
  kable_styling(bootstrap_options = "striped", full_width = F)
```

### Variable de Estudio 2: Estado del Paciente
...
kbl(tabla_estado, 
    col.names = c("Estado", "Frecuencia ($f_i$)", "Probabilidad ($P$)", "%"),
    align = "c", caption = "Probabilidad Empírica: Estado Final") |>
  kable_styling(bootstrap_options = "striped", full_width = F)
```

## Probabilidad Condicional

En esta sección definimos dos eventos específicos ($A$ y $B$) para analizar su independencia y aplicar el Teorema de Bayes utilizando la **Probabilidad Total**.

-   **Evento** $A$: Seleccionar un paciente que **Fallece** (Variable `ESTADO` = "Muerto").

-   **Evento** $B$: Seleccionar un paciente que tiene **Riesgo Crítico** (Variable `RIESGO_CLINICO` = "Crítico (9-10)").

```{r}
# --- CÁLCULOS USANDO TUS VARIABLES (ESTADO y RIESGO) ---
n_total <- nrow(DATOS_FINALES)

# 1. Probabilidades Marginales
# Usamos filter() y nrow() como es la buena práctica
f_A <- DATOS_FINALES |> filter(ESTADO == "Muerto") |> nrow()
P_A <- f_A / n_total

f_B <- DATOS_FINALES |> filter(RIESGO_CLINICO == "Crítico (9-10)") |> nrow()
P_B <- f_B / n_total

# 2. Intersección (Probabilidad Conjunta P(A y B))
f_interseccion <- DATOS_FINALES |> 
  filter(ESTADO == "Muerto", RIESGO_CLINICO == "Crítico (9-10)") |> 
  nrow()
P_Interseccion <- f_interseccion / n_total

# 3. Probabilidad Condicional Directa P(A|B) (Verosimilitud)
# Nota: Si f_B es 0, esto es 0.
...
-   **Probabilidad Conjunta (Observada):**

    $$P(A \cap B) = `r round(P_Interseccion, 4)`$$

-   **Producto de Marginales (Teórica):**
...
Deseamos calcular la probabilidad a posteriori $P(B|A)$: *Si sabemos que un paciente ha fallecido, ¿cuál es la probabilidad de que perteneciera al grupo de Riesgo Crítico?*
...
# Esto demuestra al profesor que aplicamos la fórmula de Probabilidad Total
...
$$P(B|A) = \frac{P(A|B)\cdot P(B)}{P(A_{Total})} =
\frac{`r round(P_A_dado_B,4)` \cdot `r round(P_B,4)`}{`r round(P_Total_Calculada,4)`} =
`r round((P_A_dado_B * P_B)/P_Total_Calculada,4)`$$

> **Interpretación:** Dado que el paciente falleció, la probabilidad de que proviniera del grupo de **Riesgo Crítico** es del **`r round(((P_A_dado_B * P_B)/P_Total_Calculada)*100, 2)`%**.

## Variables Aleatorias Discretas

### Variable Discreta 1: ESTADO (Modelo Bernoulli)

Analizamos la variable `ESTADO` ("Vivo" vs "Muerto"). Al ser dicotómica, proponemos el modelo Bernoulli.

-   **Variable Aleatoria** $Y$: Estado final del paciente al egreso.
-   **Rango:** $R_Y = \{0, 1\}$
-   **Modelo Propuesto:** $Y \sim Bernoulli(p)$
-   **Codificación:** $1=$ Sobrevive ("Vivo"), $0=$ Fallece ("Muerto").
...
points(as.numeric(names(tabla_b)), as.vector(tabla_b), pch = 19, col = "blue")
axis(1, at = c(0, 1), labels = c("0 (Muerto)", "1 (Vivo)"))
```

#### Validación Numérica

Comparamos la varianza empírica con la teórica ($\sigma^2 = p(1-p)$).

-   **Media Estimada (**$p$): `r round(p_hat, 4)`

-   **Varianza Teórica:** `r round(p_hat * (1 - p_hat), 4)`

-   **Varianza Empírica:** `r round(var(df_bernoulli$Y), 4)`

### Variable Discreta 2: NIVEL DE RIESGO (Candidata a Poisson)

Analizamos la severidad del riesgo clínico como una variable de conteo/ordinal.

-   **Variable Aleatoria** $X$: Nivel de severidad del riesgo.

-   **Rango:** $R_X = \{0, 1, 2, 3, 4, 5\}$

-   **Modelo Candidato:** $X \sim Poisson(\lambda)$
...
#### Comparación Gráfica (Empírica vs Teórica)

Superponemos las probabilidades reales (Azul) con las esperadas por Poisson (Rojo).
...
points(x_vals, y_emp, pch = 19, col = "blue")

# Líneas Teóricas (Desplazadas ligeramente para visualización)
lines(x_vals + 0.1, y_teor, type = "h", lwd = 3, col = "red", lty = 2)
points(x_vals + 0.1, y_teor, pch = 17, col = "red")

legend("topright", legend = c("Empírica (Real)", "Teórica (Poisson)"), 
       col = c("blue", "red"), lty = c(1, 2), pch = c(19, 17))
```

#### Validación y Conclusión

Verificamos el supuesto de equidispersión ($E[X] \approx Var(X)$).

-   **Media (**$\lambda$): `r round(lambda_est, 4)`

-   **Varianza (**$S^2$): `r round(var(df_poisson$X, na.rm=TRUE), 4)`

## Variables Aleatorias Continuas

### Variable Continua 1: EDAD (Candidata a Normal)

Evaluamos si la edad de los pacientes sigue una distribución Normal (Campana de Gauss).

-   **Variable:** Edad (años).

-   **Modelo Candidato:** $X \sim Normal(\mu, \sigma^2)$

```{r}
#| warning: false
# Histograma con Curva Normal Teórica
hist(DATOS_FINALES$EDAD, prob = TRUE, col = "lightblue", 
     main = "Distribución de la Edad", xlab = "Edad", breaks = 20)

# Curva Normal Teórica (Rojo)
curve(dnorm(x, mean=mean(DATOS_FINALES$EDAD, na.rm=TRUE), 
            sd=sd(DATOS_FINALES$EDAD, na.rm=TRUE)), 
      add=TRUE, col="red", lwd=2)
```

#### Validación Numérica

Comparación de estadísticos clave para la Normalidad.

```{r}
media_edad <- mean(DATOS_FINALES$EDAD, na.rm=TRUE)
mediana_edad <- median(DATOS_FINALES$EDAD, na.rm=TRUE)

data.frame(Estadistico = c("Media", "Mediana"), Valor = c(media_edad, mediana_edad))
```

Validación Gráfica (QQ-Plot)

```{r}
qqnorm(DATOS_FINALES$EDAD, main = "QQ-Plot Normal: Edad", pch=19, col="gray")
qqline(DATOS_FINALES$EDAD, col="red", lwd=2)
```

### Variable Continua 2: TIEMPO DE ESTANCIA (Candidata a Exponencial)

Analizamos el tiempo de hospitalización, esperando que siga una distribución Exponencial (común en tiempos de espera/duración).

-   **Variable:** Tiempo de Estancia (días).

-   **Modelo Candidato:** $X \sim Exponencial(\lambda)$

-   **Parámetro:** $\lambda = 1 / \mu$

```{r}
# --- AJUSTA AQUÍ EL NOMBRE DE TU VARIABLE SI ES DIFERENTE ---
datos_tiempo <- DATOS_FINALES$TIEMPO_ESTANCIA 
# -----------------------------------------------------------

# Estimación del parámetro lambda
media_tiempo <- mean(datos_tiempo, na.rm = TRUE)
lambda_estancia <- 1 / media_tiempo
```

#### Exploración Gráfica (Histograma + Curva Teórica)

La distribución Exponencial tiene una forma decrescente característica.

```{r}
# Histograma
hist(datos_tiempo, prob = TRUE, col = "#ffcc99", border = "white",
     main = "Distribución Tiempo de Estancia", 
     xlab = "Días de Estancia", breaks = 30)

# Curva Exponencial Teórica (Rojo)
curve(dexp(x, rate = lambda_estancia), 
      add = TRUE, col = "red", lwd = 2, lty = 1)

legend("topright", legend = c("Datos Reales", "Modelo Exponencial"),
       fill = c("#ffcc99", NA), border = c("white", NA),
       lwd = c(NA, 2), col = c(NA, "red"))
```

#### Validación Numérica (Propiedad de Memoria)

En una distribución exponencial ideal, la desviación estándar es igual a la media ($\sigma = \mu$).

```{r}
sd_tiempo <- sd(datos_tiempo, na.rm = TRUE)

validacion_exp <- data.frame(
  Medida = c("Media (Esperanza)", "Desviación Estándar"),
  Valor = c(media_tiempo, sd_tiempo),
  Diferencia = abs(media_tiempo - sd_tiempo)
)
print(validacion_exp)
```
...
3.  **Equidad en el sistema de vigilancia:** La distribución por sexo muestra un moderado predominio masculino (57.3%), alineado con patrones epidemiológicos nacionales de COVID-19 en México durante 2021, aunque la letalidad aparente por sexo en esta muestra fue similar entre ambos grupos.
...
5.  **Dependencia Estadística de la Mortalidad:** La prueba de probabilidad condicional demostró que el fallecimiento y el riesgo crítico **no son eventos independientes** ($P(A \cap B) \neq P(A)P(B)$). Esto confirma estadísticamente que la acumulación de factores de riesgo (comorbilidades) altera significativamente la probabilidad de supervivencia del paciente.

6.  **Modelado de Supervivencia (Bernoulli):** La variable *Estado del Paciente* se ajusta perfectamente al modelo Bernoulli. La validación numérica mostró que la varianza empírica coincide con la teórica ($\sigma^2 = p(1-p)$), lo que valida el uso de este modelo para describir la probabilidad de éxito (supervivencia) en la población.

7.  **Rechazo del Modelo de Conteo Simple (Poisson):** El *Nivel de Riesgo Clínico* no se comporta como una variable Poisson debido a la presencia de **sobredispersión** (la varianza es mayor que la media) y a la naturaleza acotada de la escala. Esto sugiere que la acumulación de riesgos no es un proceso puramente aleatorio, sino que está influenciado por factores estructurales biológicos.

8.  **Comportamiento de la Edad (Aproximación Normal):** La distribución de la edad de los pacientes tiende a seguir una distribución Normal (campana de Gauss), aunque con desviaciones en los extremos (colas) visibles en el QQ-Plot. Esto es consistente con fenómenos demográficos naturales, permitiendo el uso de modelos gaussianos con cierta precaución.

9.  **Dinámica de la Hospitalización (Exponencial):** El *Tiempo de Estancia* exhibe un patrón de decaimiento que se ajusta razonablemente al modelo Exponencial. La similitud entre la media y la desviación estándar respalda la propiedad de "falta de memoria" de esta distribución, útil para modelar tiempos de servicio en sistemas de salud.
