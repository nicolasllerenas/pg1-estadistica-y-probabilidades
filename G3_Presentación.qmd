...
Siguiendo las indicaciones del docente, analizamos el comportamiento de **dos variables de estudio**, definiendo formalmente el experimento y calculando sus probabilidades a partir de las frecuencias observadas en la muestra ($n = 17,714$).
...
> **Conclusión:** Dado que $`r round(P_Interseccion, 4)` \neq `r round(P_A * P_B, 4)`$, los eventos **NO son independientes**. Existe dependencia entre el nivel de riesgo y el fallecimiento.
...
*(Este valor coincide con la probabilidad marginal observada, validando el cálculo).*
